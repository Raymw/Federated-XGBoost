...
struct TrainParam : public dmlc::Parameter<TrainParam> {
  ...
  // used for federated learning model, the maximum Index of Aggregated set used for split
  int fl_split;

  // declare the parameters
  DMLC_DECLARE_PARAMETER(TrainParam) {
    DMLC_DECLARE_FIELD(fl_split)
        .set_lower_bound(0)
        .set_default(0)
        .describe("maximum Index of Aggregated set used for split(Federated learning).");
